...
public  class OpenAiChatModelParams implements IModelParams {
    ...
    public List<BaseField> toForm() {
        ...
        BaseField maxTokens=new SliderField(1,1024000,1,0,"输出最大Tokens",ParamKey.MAX_TOKENS,"指定模型可生成的最大token个数",1024);
        BaseField switchField=new SwitchField("思考模式",ParamKey.ENABLE_THINKING,"在输出最终回答之前，模型会先输出一段思维链内容，以提升最终答案的准确性。",true);
