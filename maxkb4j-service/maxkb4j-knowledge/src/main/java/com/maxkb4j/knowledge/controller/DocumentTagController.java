...
public class DocumentTagController {
    ...
    public R<List<DocumentTagEntity>> docTags(@PathVariable("id") String id,  @PathVariable String docId,@RequestParam String name) {
        return R.success(documentTagService.list());
    }


    @SaCheckPerm(PermissionEnum.KNOWLEDGE_DOCUMENT_DELETE)
    @DeleteMapping("/knowledge/{id}/document/{docId}/tags/batch_delete")
