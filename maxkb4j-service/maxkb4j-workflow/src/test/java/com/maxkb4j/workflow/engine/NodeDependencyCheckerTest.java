...
class NodeDependencyCheckerTest {
    ...
    @Test
    void isSkipNode_returnsTrueWhenAllUpstreamSkipped() {
        AbsNode a = withStatus(node("a"), NodeStatus.SKIP);
        AbsNode b = withStatus(node("b"), NodeStatus.SKIP);
        AbsNode c = node("c");
        NodeDependencyChecker checker = checker(List.of(a, b, c), List.of(edge("a", "c"), edge("b", "c")));

        assertThat(checker.isSkipNode(c)).isTrue();
    }

    @Test
    void isSkipNode_returnsFalseWhenAnyUpstreamNotSkipped() {
        AbsNode a = withStatus(node("a"), NodeStatus.SKIP);
        AbsNode b = withStatus(node("b"), NodeStatus.SUCCESS);
        AbsNode c = node("c");
        NodeDependencyChecker checker = checker(List.of(a, b, c), List.of(edge("a", "c"), edge("b", "c")));

        assertThat(checker.isSkipNode(c)).isFalse();
    }

    @Test
    void isSkipNode_noUpstream_returnsTrue() {
        AbsNode a = node("a");
        NodeDependencyChecker checker = checker(List.of(a), List.of());

        assertThat(checker.isSkipNode(a)).isTrue();
    }

    @Test
    void isSkipNodeWithExclude_ignoresExcludedUpstream() {
        AbsNode a = withStatus(node("a"), NodeStatus.SUCCESS);
        AbsNode b = withStatus(node("b"), NodeStatus.SKIP);
        AbsNode c = node("c");
        NodeDependencyChecker checker = checker(List.of(a, b, c), List.of(edge("a", "c"), edge("b", "c")));

        // 排除 a 后剩余上游 b 为 SKIP，可跳过
        assertThat(checker.isSkipNode(c, "a")).isTrue();
        // 排除 b 后剩余上游 a 未 SKIP，不可跳过
        assertThat(checker.isSkipNode(c, "b")).isFalse();
    }

    @Test
    void isSkipNodeWithExclude_returnsTrueWhenNoOtherUpstreamRemains() {
        AbsNode a = withStatus(node("a"), NodeStatus.SUCCESS);
        AbsNode b = node("b");
        NodeDependencyChecker checker = checker(List.of(a, b), List.of(edge("a", "b")));

        // b 的唯一上游 a 被排除后，视为可跳过
        assertThat(checker.isSkipNode(b, "a")).isTrue();
    }
}
