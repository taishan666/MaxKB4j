...
- 📄 [MaxKB4j 商业价值白皮书](./docs/MaxKB4j-商业价值白皮书.md)
- 🧪 [MaxKB4j 回归测试报告](./docs/MaxKB4j-回归测试报告.md)
...
| 🧠 AgenticRAG 与 Advanced RAG | • **Advanced RAG**：支持向量检索、全文检索与混合检索（多路召回），内置 Reranker 重排序节点对多路召回结果二次精排，可设置 Top-K、相似度阈值等参数，显著提升召回准确率<br>• **AgenticRAG**：依托可视化工作流编排，结合意图识别、知识库搜索、重排序、条件分支等节点，由智能体动态决策检索路径与推理流程，支持多跳问答与复杂任务的自动拆解，适配高阶问答场景 |
